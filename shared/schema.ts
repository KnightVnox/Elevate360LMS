...
export const courseModules = pgTable("course_modules", {
  ...
  scormScoId: varchar("scorm_sco_id"), // Reference to SCORM SCO if imported
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const scormPackages = pgTable("scorm_packages", {
  ...
  title: varchar("title").notNull(),
  version: varchar("version").notNull(), // '1.2' or '2004'
  manifestData: jsonb("manifest_data").notNull(),
  packageUrl: varchar("package_url"), // URL to stored ZIP file
  organizationIdentifier: varchar("organization_identifier"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

// SCORM SCOs (Sharable Content Objects)
export const scormScos = pgTable("scorm_scos", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  packageId: varchar("package_id").notNull().references(() => scormPackages.id, { onDelete: 'cascade' }),
  identifier: varchar("identifier").notNull(),
  title: varchar("title").notNull(),
  launchUrl: varchar("launch_url").notNull(),
  metadata: jsonb("metadata"),
  orderIndex: integer("order_index").notNull(),
  entryPoint: varchar("entry_point"), // Entry point HTML file
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const scormTracking = pgTable("scorm_tracking", {
  ...
  scoId: varchar("sco_id").notNull().references(() => scormScos.id, { onDelete: 'cascade' }),
  cmiData: jsonb("cmi_data").notNull(), // Complete CMI data
  completionStatus: varchar("completion_status"), // 'completed', 'incomplete', 'not attempted', 'unknown'
  successStatus: varchar("success_status"), // 'passed', 'failed', 'unknown'
  scoreScaled: decimal("score_scaled", { precision: 5, scale: 4 }),
  scoreRaw: integer("score_raw"),
  scoreMin: integer("score_min"),
  scoreMax: integer("score_max"),
  sessionTime: integer("session_time"), // in seconds
  totalTime: integer("total_time"), // in seconds
  location: varchar("location"), // Last position in content
  suspendData: text("suspend_data"), // User's suspend data
  lastAccessed: timestamp("last_accessed").defaultNow(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
  index("idx_scorm_tracking_user_sco").on(table.userId, table.scoId),
]);
...
export const coursesRelations = relations(courses, ({ one, many }) => ({
  ...
  scormPackages: many(scormPackages),
}));
...
export const scormPackagesRelations = relations(scormPackages, ({ one, many }) => ({
  course: one(courses, {
    fields: [scormPackages.courseId],
    references: [courses.id],
  }),
  scos: many(scormScos),
}));

export const scormScosRelations = relations(scormScos, ({ one, many }) => ({
  package: one(scormPackages, {
    fields: [scormScos.packageId],
    references: [scormPackages.id],
  }),
  tracking: many(scormTracking),
}));

export const scormTrackingRelations = relations(scormTracking, ({ one }) => ({
  user: one(users, {
    fields: [scormTracking.userId],
    references: [users.id],
  }),
  sco: one(scormScos, {
    fields: [scormTracking.scoId],
    references: [scormScos.id],
  }),
}));
...
export const insertSCORMScoSchema = createInsertSchema(scormScos).omit({
  id: true,
  createdAt: true,
});

export const insertSCORMTrackingSchema = createInsertSchema(scormTracking).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
  lastAccessed: true,
...
export type SCORMSco = typeof scormScos.$inferSelect;
export type SCORMTracking = typeof scormTracking.$inferSelect;
export type InsertSCORMPackage = z.infer<typeof insertSCORMPackageSchema>;
export type InsertSCORMSco = z.infer<typeof insertSCORMScoSchema>;
export type InsertSCORMTracking = z.infer<typeof insertSCORMTrackingSchema>;
