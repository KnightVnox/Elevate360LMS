...
export interface IStorage {
  ...
  getSCORMPackagesByCourse(courseId: string): Promise<SCORMPackage[]>;
  createSCORMSco(scoData: InsertSCORMSco): Promise<SCORMSco>;
  getSCORMScosByPackage(packageId: string): Promise<SCORMSco[]>;
  getSCORMSco(id: string): Promise<SCORMSco | undefined>;
  createSCORMTracking(trackingData: InsertSCORMTracking): Promise<SCORMTracking>;
  updateSCORMTracking(id: string, updates: Partial<InsertSCORMTracking>): Promise<SCORMTracking | undefined>;
  getSCORMTracking(userId: string, scoId: string): Promise<SCORMTracking | undefined>;
  getSCORMTrackingByUser(userId: string, courseId?: string): Promise<SCORMTracking[]>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async createSCORMPackage(packageData: InsertSCORMPackage): Promise<SCORMPackage> {
    const [pkg] = await db.insert(scormPackages).values(packageData).returning();
    return pkg;
  }
  ...
  async getSCORMPackagesByCourse(courseId: string): Promise<SCORMPackage[]> {
    return await db.select().from(scormPackages).where(eq(scormPackages.courseId, courseId));
  }

  async createSCORMSco(scoData: InsertSCORMSco): Promise<SCORMSco> {
    const [sco] = await db.insert(scormScos).values(scoData).returning();
    return sco;
  }

  async getSCORMScosByPackage(packageId: string): Promise<SCORMSco[]> {
    return await db
      .select()
      .from(scormScos)
      .where(eq(scormScos.packageId, packageId))
      .orderBy(asc(scormScos.orderIndex));
  }

  async getSCORMSco(id: string): Promise<SCORMSco | undefined> {
    const [sco] = await db.select().from(scormScos).where(eq(scormScos.id, id));
    return sco;
  }

  async createSCORMTracking(trackingData: InsertSCORMTracking): Promise<SCORMTracking> {
    const [tracking] = await db.insert(scormTracking).values(trackingData).returning();
    return tracking;
  }

  async updateSCORMTracking(id: string, updates: Partial<InsertSCORMTracking>): Promise<SCORMTracking | undefined> {
    const [tracking] = await db
      .update(scormTracking)
      .set({ ...updates, updatedAt: new Date() })
      .where(eq(scormTracking.id, id))
      .returning();
    return tracking;
  }

  async getSCORMTracking(userId: string, scoId: string): Promise<SCORMTracking | undefined> {
    const [tracking] = await db
      .select()
      .from(scormTracking)
      .where(and(eq(scormTracking.userId, userId), eq(scormTracking.scoId, scoId)));
    return tracking;
  }

  async getSCORMTrackingByUser(userId: string, courseId?: string): Promise<SCORMTracking[]> {
    let query = db
      .select()
      .from(scormTracking)
      .where(eq(scormTracking.userId, userId));

    if (courseId) {
      // Join with scos and packages to filter by course
      query = db
        .select()
        .from(scormTracking)
        .innerJoin(scormScos, eq(scormTracking.scoId, scormScos.id))
        .innerJoin(scormPackages, eq(scormScos.packageId, scormPackages.id))
        .where(and(eq(scormTracking.userId, userId), eq(scormPackages.courseId, courseId))) as any;
    }
